...
(1, '无线降噪耳机 Pro', 1299.00, 1, '/img.png', '主动降噪，40小时续航，Hi-Fi 音质，舒适佩戴体验。', 50, 'on', TRUE),
(2, '智能手表 Ultra', 2499.00, 1, '/img_1.png', '健康监测、GPS 定位、50米防水，全天候智能伴侣。', 30, 'on', TRUE),
(3, '极简羊毛大衣', 1899.00, 2, '/img_2.png', '100% 澳洲美利奴羊毛，经典剪裁，温暖有型。', 20, 'on', TRUE),
(4, '真皮手提包', 1599.00, 2, '/img_4.png', '头层牛皮，手工缝制，大容量设计，商务通勤皆宜。', 15, 'on', FALSE),
(5, '北欧风台灯', 399.00, 3, '/img_3.png', '三档调光，护眼无频闪，简约设计点缀居家空间。', 80, 'on', TRUE),
(6, '记忆棉枕头', 299.00, 3, '/img_5.png', '人体工学曲线，慢回弹材质，深度睡眠从此开始。', 100, 'on', FALSE),
(7, '精华护肤套装', 899.00, 4, '/img_6.png', '三重玻尿酸配方，深层补水，焕活肌肤光泽。', 40, 'on', TRUE),
(8, '专业跑鞋', 799.00, 5, '/img_7.png', '碳板科技，轻量回弹，马拉松级缓震保护。', 60, 'on', FALSE),
(9, '露营帐篷 4人', 1299.00, 5, '/img_8.png', '防雨防风，快速搭建，户外露营的理想选择。', 25, 'on', FALSE),
(10, '设计思维经典', 68.00, 6, '/img_9.png', 'IDEO 设计方法论经典之作，激发创意灵感。', 200, 'on', FALSE),
(11, '机械键盘 RGB', 599.00, 1, '/img_10.png', '青轴手感，全键无冲，1680万色 RGB 灯效。', 45, 'off', FALSE),
(12, '真丝围巾', 459.00, 2, '/img_7.png', '100% 桑蚕丝，手工印花，四季百搭单品。', 35, 'on', FALSE);
...
(1, 1, '202601010001', TIMESTAMP '2026-01-01 10:00:00', TIMESTAMP '2026-01-01 10:05:00', 3, 1299.00, '北京市朝阳区建国路 88 号', '演示用户', '13800138000', '顺丰速运', 'SF1234567890', '已签收');
...
-- 同步自增序列，避免后续 INSERT 主键冲突
SELECT setval(pg_get_serial_sequence('category', 'id'), COALESCE((SELECT MAX(id) FROM category), 1));
SELECT setval(pg_get_serial_sequence('good', 'id'), COALESCE((SELECT MAX(id) FROM good), 1));
SELECT setval(pg_get_serial_sequence('"user"', 'id'), COALESCE((SELECT MAX(id) FROM "user"), 1));
SELECT setval(pg_get_serial_sequence('admin', 'id'), COALESCE((SELECT MAX(id) FROM admin), 1));
SELECT setval(pg_get_serial_sequence('shop_order', 'id'), COALESCE((SELECT MAX(id) FROM shop_order), 1));
SELECT setval(pg_get_serial_sequence('order_item', 'id'), COALESCE((SELECT MAX(id) FROM order_item), 1));
SELECT setval(pg_get_serial_sequence('carousel', 'id'), COALESCE((SELECT MAX(id) FROM carousel), 1));
